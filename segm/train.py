...
@click.option("--pretrained-params-file", default=None, type=str)
def main(
    log_dir,
    dataset,
    im_size,
    ...
    model = mrestnet.transformer.Transformer(input_channels=3, number_of_classes=n_cls, input_shape=(768, 768), encoder_heads=3,
                                           decoder_heads=3,
                                           patch_size=16, encoder_layers=12, decoder_layers=2,
                                           h_size=768, model_dimensionality=192)
    model.to(ptu.device)

    if pretrained_params_file is None:
        print('Please specify a pretrained params file')
        sys.exit(0)
    #pretrained_params_file = '/data/ViT/pretrained_models/Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0.0--imagenet2012-steps_20k-lr_0.03-res_384.npz'
    params = np.load(pretrained_params_file)
    keys, values = zip(*list(params.items()))
    weights_dictionary = dict(zip(keys, values))

    # Remove classification head from pretrained parameters
    weights_dictionary.pop('head/bias', None)
    weights_dictionary.pop('head/kernel', None)

    model.load_pretrained(weights_dictionary)

    # optimizer
    optimizer_kwargs = variant["optimizer_kwargs"]
    optimizer_kwargs["iter_max"] = len(
        train_loader) * optimizer_kwargs["epochs"]
    ...
    print(
        f"Unet Decoder parameters: {num_params(model_without_ddp.unet_decoder)}")
    print(
        f"Vit Decoder parameters: {num_params(model_without_ddp.transformer_decoder)}")
    print(
        f"Scaler parameters: {num_params(model_without_ddp.scaler)}")
    print(
        f"Fuser parameters: {num_params(model_without_ddp.fuser)}")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if ptu.dist_rank == 0:
            ...
            if eval_epoch and miou > best_miou:
                best_miou = miou
                print(f'Saving best model: {best_miou}')
